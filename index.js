...
app.patch("/booked/:tutorId", verifyToken, async (req, res) => {
  ...
  if (tutor.totalSlot <= 0) {
    return res.status(400).json({
      message: "This session is fully booked. You can not join at the moment.",
    });
  }
...
app.patch("/booked/cancel/:id", verifyToken, async (req, res) => {
  const { id } = req.params;
  ...
  if (existingBooked.status === "cancelled") {
  ...
  const result = await tutorCollection.updateOne(
    { _id: new ObjectId(existingBooked.tutorId) },
    { $inc: { totalSlot: 1 } }
  );
